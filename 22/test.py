...
(S := [*stdin.readlines()]) and (
    ...
    n := lambda x, d: [(x, -d * 1j), (x - d * 1j, d), (x + d * (1 - 1j), d * 1j)][
        c(x, d)
    ]
) and (
    ...
    and g.update({x: (y[0], -y[1]), y: (x[0], -x[1])})
    or (c(*y), n(*y))
) and f(
    next(
        (x, d)
    ...
    x := reduce(
        ...
        if p in "RL"
        else reduce(lambda y, _: j(*y) if m[j(*y)[0]] else y, range(int(p)), x),
        findall("\d+|R|L", S[-1]),
        (min(m, key=lambda x: (x.real, x.imag)), 1j),
    )
) and print(
    1000 * x[0].real + 4 * x[0].imag + 1004 + [1j, 1, -1j, -1].index(x[1])
)
